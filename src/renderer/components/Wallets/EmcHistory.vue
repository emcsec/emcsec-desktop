    ...
    <div class="history">
        <table class="table small">
            ...
            <tr>
                ...
                <th class="text-right">{{ $I18n.translate('amount') }} (EMC)</th>
            </tr>
            </thead>
                    ...
                    <div class="modal-body small">
                        <table class="table">
                            ...
                            <tr>
                                ...
                                <td>
                                    <span v-if="details.type === 'out'">-{{details.amount}} EMC</span>
                                    <span v-else>{{details.amount}} EMC</span>
                                </td>
                            </tr>
                            <tr v-if="details.type !== 'received'">
                                <td>{{ $I18n.translate('fee') }}:</td>
                                <td>-{{ details.fee }} EMC</td>
                            </tr>
                            <tr>
  ...
  export default {
    ...
    mounted () {
      ...
      this.$watch('$store.state.RemoteData.data', function () {
        if (typeof this.getData().walletId !== 'undefined' && this.getData().walletId !== this.data.walletId && this.getData().type === 'emc') {
      ...
      setHistory (history) {
        ...
        for (let [i, h] of history.pending.entries()) {
          this.$set(this.pending, i, {
            ...
            fee: typeof h.fee === 'string' ? h.fee : h.fee / 1000000,
            address: h.address,
            tx_hash: h.tx_hash,
            type: h.tx_type,
            height: h.height
